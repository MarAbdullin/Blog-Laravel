...
class PostController extends Controller
{
    ...
    public function store(PostRequest $request)
    {
        ...
        return redirect()->route('user.post.show', ['post' => $post->id])->with('success', 'Новый пост успешно создан');
    ...
    {   
        ...
        return view('user.post.show', compact('comments', 'post'));        
    }
    ...
    {
        // можно обновить только свои посты
        if(! $post->isAuthor()){
            abort(404);
        }

        // обновить можно не опубликованные посты
        if($post->isVisible()){
            abort(404);
        }
        ...
        return redirect()->route($route, $param)->with(['success' => 'Пост успешно обновлен']);
    ...
    public function destroy(Post $post)
    {
        ...
        // можно удалять не опубликованные посты
        if($post->isVisible()){
            abort(404);
        }
        ...
        return redirect()->route('user.post.index')->with(['success' => 'Пост успешно удалён']);
    }
